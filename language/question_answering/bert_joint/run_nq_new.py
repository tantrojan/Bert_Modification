...
def addAllSentences(dirpath, filename):
    document_sentences = []
    json_obj = json.loads(open(os.path.join(dirpath, filename)).read())

    sections = json_obj['metadata']['sections']
    if (sections != None):
        for section in sections:
            if (section['heading'] != None and
                    'Introduction' not in section['heading'] and
                    'Related Work' not in section['heading'] and
                    'Conclusion' not in section['heading'] and
                    'Acknowledgement' not in section['heading'] and
                    'Model' not in section['heading']):

                sentences = nltk.sent_tokenize(section['text'].lower())
...
def read_gold_set(dirpath, file):
    f = open(os.path.join(dirpath, file), "r")
    gold_sentences = f.read().split(',')
    return list(map(int, gold_sentences))

def caption_sentences(dirpath, file):
    cap_sentences = []
    f = open(os.path.join(dirpath, file), "r")
    ...
    for index, sentence in enumerate(sentences):
        ...
        if (len(sent_split) > 1):
            cap_sentences.append(sent_split[1])
        else:
            cap_sentences.append(sentence)
    f.close()
...
############################################################ Similar to read_nq_examples()
def fetch_data(is_training):

  document_sentences= None
  cap_sentences = None

  # Directory paths for all_text, Table_caption, and Relevant Sentences
  dirpath = "/home/gosh/Documents/Extraction/All_text/"
  cap_dir = "/home/gosh/Documents/Extraction/Table_caption"
  rel_dir = "/home/gosh/Documents/Extraction/Table_caption"

  doc_sent_dict = {} #All document sentences
  cap_sent_dict = {} #All caption sentences
  gold_dataset_dict = {} #All relevant sentences


  ########## Extracting all sentences from a file
  for (dirpath, dirnames, filenames) in os.walk(dirpath):
      filenames.sort(key=lambda f: int(''.join(filter(str.isdigit, f))))

      for filename in filenames:
          document_sentences = addAllSentences(dirpath, filename)
          doc_sent_dict[os.path.basename(filename).split('.')[0]]=document_sentences
        # doc_sent_dict[os.path.splitext(os.path.basename(os.path.splitext(os.path.basename(filename))[0]))[0]]=document_sentences

 ###############################################



  ########## Extracting captions from a file
  for (cap_dir, cap_dirnames, cap_filenames) in os.walk(cap_dir):
      cap_filenames.sort(key=lambda f: int(''.join(filter(str.isdigit, f))))
      for cap_filename in cap_filenames:
          if ('caption' in cap_filename):
              cap_sentences = caption_sentences(cap_dir, cap_filename)
              one_cap_sentence = " ".join(cap_sentences)
              one_cap_sentence_clean = one_cap_sentence.translate(None, string.punctuation)
              cap_sent_dict[os.path.basename(cap_filename).split('|')[0]] = one_cap_sentence_clean
  ##########################################

  actual_map = []
  ########## Extracting relevant sentence numbers from a file
  for (rel_dir, rel_dirnames, rel_filenames) in os.walk(rel_dir):
      rel_filenames.sort(key=lambda f: int(''.join(filter(str.isdigit, f))))
      for rel_filename in rel_filenames:
          if('rel' in rel_filename):
              relevant_sentences = read_gold_set(rel_dir, rel_filename)
              actual_map.append(relevant_sentences)
              gold_dataset_dict[rel_filename.split('|')[0]] = relevant_sentences
  ###########################################################

  input_data = []
  count = 0

  for k1,v1 in doc_sent_dict.items():
    for k2,v2 in cap_sent_dict.items():
      if k1 in k2:
        for k3,v3 in gold_dataset_dict.items():
          if k2 == k3 :
              all_sents, caption, relevant_sents = v1, v2, v3
              input_data.append(create_example_from_file(count,all_sents,caption,relevant_sents))
              count=count+1
...
# Example has ['has_correct_context', 'name', 'contexts', 'answers', 'contexts_map', 'questions', 'id']
def create_example_from_file(ID, all_sents, caption, relevant_sents):
  ...
  for i,sent in enumerate(all_sents):
    for j,word in enumerate(list(sent.split(' '))):
      contexts_map.append(k)
      k=k+1
      contexts = contexts + " " + word
      if i in relevant_sents:
        if span_start==-1:
          span_start=k
        relevant = relevant + " " + word
        span_end=k+len(list(sent.split(' ')))





  question = {"input_text": caption}
  answer = {
      "candidate_id": relevant_sents[0],
      "span_text": relevant,
...
def main(_):
  ...
  print("\nn111111111111111111111111111111111111111111111111111111111111111111111111111111111111\n")
  ...
  if FLAGS.do_predict:
    ...
    ############################################### Modification made
    eval_examples = fetch_data(is_training=False)
